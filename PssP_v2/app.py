...
mysql_username = os.getenv("MYSQL_USER_localdb")
mysql_password = os.getenv("MYSQL_PASSWORD_localdb")
mysql_host = os.getenv("MYSQL_HOSTNAME_localdb")
...
#### Initial setup 
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] =f'mysql+mysqldb://{mysql_username}:{mysql_password}@{mysql_host}:3306/{mysql_database}'
...
def get_gui_patients():
    if 'loggedin' in session and session['account_type'] == 'provider':
    ...
    elif 'loggedin' in session and session['account_type'] == 'admin':   #Should mins be able to actually edit real patients is up for debate
        returned_Patients = Patients.query.all() # documentation for .query exists: https://docs.sqlalchemy.org/en/14/orm/query.html
        return render_template("patient_all.html", patients = returned_Patients)
    else:
        return redirect(url_for('get_patient_details', mrn=session['mrn']))
